...
def query_pinecone_user(query_string,index, top_k=10, namespace="",filter={"user_id": "fake_user_id"}):
    xc = get_embedding(query_string)
    result = user_index.query(vector=xc, top_k=top_k, include_metadata=True, namespace=namespace,filter=filter)
    return result
...
#     print(result)
#     return result
def query_pinecone_book(query_string,index, top_k=1, namespace="default-namespace"):
    """
    ...
    top_index = int(top_match.id)  # Assuming the IDs are integers or convertible to integers
    ...
    for index in indices_to_fetch:
        ...
        individual_result = book_index.fetch(ids=[str(index)], namespace=namespace)
